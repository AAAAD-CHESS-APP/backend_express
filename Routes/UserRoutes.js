...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified) {
      return res.status(401).json({ redirect: "http://localhost:5173/verify" });
    }
...
router.post("/verifytokenAndGetUsername", async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified)
      return res.status(400).send({ error: "Your Email is not verfied !" });
...
router.post("/resetPasswordToken", async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(400).send({ error: "Email is not registered with us" });
    const token = jwt.sign(
...
router.patch("/user/password/:id", Auth, async (req, res) => {
  try {
    const { id } = req.params;
    const { password, confirmPass } = req.body;
    if (req.userId !== id) {
...
`,
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res
        .status(200)
        .send({ message: "Verification email sent. Check your inbox." });
    });
  } catch (err) {
...
router.post("/request-unban/:playerId", async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(400).send("Invalid User Id");
    if (!user.isBanned)
      return res.status(200).send({ message: "You are not banned" });
